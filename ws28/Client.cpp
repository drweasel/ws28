...
std::string
base64_encode(const unsigned char *input, std::size_t length)
{
...
struct DataFrameHeader
{
    ...
    uint8_t opcode() { return data[0] & 0x0F; }

    uint8_t len() { return data[1] & 0x7F; }
};
...
template< size_t N >
void
Client::WriteRaw(uv_buf_t bufs[N])
{
    if (!m_Socket)
        return;
...
void
Client::WriteRawQueue(std::unique_ptr< char[] > data, size_t len)
{
    if (!m_Socket)
        return;
...
void
Client::Write(const char *data, size_t len)
{
    uv_buf_t bufs[1];
...
void
Client::WriteDataFrameHeader(uint8_t opcode, size_t len, char *headerStart)
{
...
size_t
Client::GetDataFrameHeaderSize(size_t len)
{
    if (len >= 126)
    {
        if (len > UINT16_MAX)
...
void
Client::OnRawSocketData(char *data, size_t len)
{
    if (len == 0)
        return;
    if (!m_Socket)
    ...
    if (m_bWaitingForFirstPacket)
    {
        ...
        if (
          m_pServer->GetSSLContext() != nullptr &&
          (data[0] == 0x16 || uint8_t(data[0]) == 0x80))
        {
            if (
              m_pServer->m_fnCheckTCPConnection &&
    ...
    if (IsSecure())
    {
        if (!m_pTLS->ReceivedData(
              data,
              len,
              [&](char *data, size_t len) { OnSocketData(data, len); }))
        {
            return Destroy();
        }
...
void
Client::OnSocketData(char *data, size_t len)
{
    ...
    auto Bail = [&]()
    {
        ...
        if (usingLocalBuffer)
        {
            if (!buffer.empty())
            {
                ...
                m_Buffer.insert(
                  m_Buffer.end(), buffer.data(), buffer.data() + buffer.size());
            }
        }
        else
        {
    ...
    if (
      !m_bHasCompletedHandshake && m_pServer->GetAllowAlternativeProtocol() &&
      buffer[0] == 0x00)
    {
        ...
        if (
          !m_pServer->m_fnCheckAlternativeConnection ||
    ...
    else if (!m_bHasCompletedHandshake)
    {
...

        {
            ...
            assert(
              endOfLine !=
              std::string_view::npos); // Can't fail because of a check above
        ...
        HTTPRequest req{
            m_pServer, method, path, m_IP, headers,
        };
    ...
    for (;;)
    {
        ...
        if (m_bUsingAlternativeProtocol)
        {
            if (buffer.size() < 4)
                return Bail();
            uint32_t frameLength = ((uint32_t)(uint8_t)buffer[0]) |
        ...
        { // Websockets
            ...
            if (frameLength == 126)
            {
                if (buffer.size() < 4)
                    return Bail();
                frameLength = (*(uint8_t *)(curPosition) << 8) |
            ...
            else if (frameLength == 127)
            {
                ...
                frameLength = ((uint64_t)*(uint8_t *)(curPosition) << 56) |
                              ((uint64_t)*(uint8_t *)(curPosition + 1) << 48) |
                              ((uint64_t)*(uint8_t *)(curPosition + 2) << 40) |
                              ((uint64_t)*(uint8_t *)(curPosition + 3) << 32) |
                              (*(uint8_t *)(curPosition + 4) << 24) |
                              (*(uint8_t *)(curPosition + 5) << 16) |
                              (*(uint8_t *)(curPosition + 6) << 8) |
                              (*(uint8_t *)(curPosition + 7) << 0);
            ...
            if (header.opcode() >= 0x08)
            {
                ...
                if (frameLength > 125)
                    return Close(
                      1002, "Control op codes can't be more than 125 bytes");
            ...
            else
            {
                ...
                if (
                  m_FrameBuffer.size() + frameLength >=
                  m_pServer->m_iMaxMessageSize)
                    return Close(1009, "Message too large");

                m_FrameBuffer.insert(
                  m_FrameBuffer.end(), curPosition, curPosition + frameLength);
...
void
Client::ProcessDataFrame(uint8_t opcode, char *data, size_t len)
{
    switch (opcode)
    {
    case 9: // Ping
...
void
Client::Close(uint16_t code, const char *reason, size_t reasonLen)
{
    if (m_bIsClosing)
        return;
...
void
Client::Send(const char *data, size_t len, uint8_t opcode)
{
    if (!m_Socket)
        return;
