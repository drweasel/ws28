...
bool
Server::Listen(int port, bool ipv4Only)
{
    if (m_Server)
        return false;
    ...
    if (
      uv_listen(
        (uv_stream_t *)server.get(),
        512,
        [](uv_stream_t *server, int status)
        { ((Server *)server->data)->OnConnection(server, status); }) != 0)
    {
        return false;
    }
...
void
Server::OnConnection(uv_stream_t *server, int status)
{
    if (status < 0)
        return;
...
Server::NotifyClientPreDestroyed(Client *client)
{
