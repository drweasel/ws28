...
{
public:
    HTTPResponse &status(int v)
    {
        statusCode = v;
        return *this;
    }
    HTTPResponse &send(std::string_view v)
    {
        body.append(v);
        return *this;
    }
...
public:
    ...
    inline bool GetAllowAlternativeProtocol()
    {
    ...
    {
        if (m_Server)
            uv_ref((uv_handle_t *)m_Server.get());
    }
    void Unref()
    {
        if (m_Server)
            uv_unref((uv_handle_t *)m_Server.get());
    }
